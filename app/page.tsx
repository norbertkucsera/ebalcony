...
export default function Home() {
  ...
  const estimate = useMemo(() => {
    const annualProduction = city === "Constanța" || city === "Craiova" ? 920 : city === "Cluj-Napoca" ? 760 : 820;
    const energyPrice = 1.05;
    const directUseRate = Math.min(0.72, 0.25 + consumption / 1000);
    const selfConsumed = Math.round(annualProduction * directUseRate);
    const saving = Math.round(selfConsumed * energyPrice);
    return { annualProduction, saving, selfConsumed, directUseRate: Math.round(directUseRate * 100), coverage: Math.min(32, Math.round((saving / (consumption * 12 * energyPrice)) * 100)) };
  }, [consumption, city]);
  ...
  return (
    <main>
      ...
      <section className="section shell intro" id="cum-functioneaza"><div className="section-label">01 / IDEEA</div><div className="intro-main"><h2>Energia face un drum scurt. De la soare, direct la tine.</h2><p>Un sistem de balcon este o mini-centrală pentru consumul tău de zi. Panourile produc, microinvertorul transformă, iar casa folosește energia înainte să o ceară din rețea.</p><div className="energy-flow"><div><span>01</span><strong>Soare</strong><small>lumina devine energie</small></div><i>→</i><div><span>02</span><strong>Panouri</strong><small>până la 800 W</small></div><i>→</i><div><span>03</span><strong>Casa ta</strong><small>consum mai mic</small></div></div><div className="wiring-diagram" aria-label="Schemă conceptuală a unui sistem solar de balcon"><div className="diagram-head"><span>SCHEMĂ CONCEPTUALĂ</span><small>flux de energie · nu instrucțiune de cablare</small></div><div className="diagram-canvas"><div className="diagram-node panels"><span className="node-icon">▦</span><strong>Panouri solare</strong><small>2 × 460 Wp</small></div><span className="diagram-wire wire-a" /><span className="diagram-arrow arrow-a">›</span><div className="diagram-node inverter"><span className="node-icon">⌁</span><strong>Microinvertor</strong><small>800 W AC</small></div><span className="diagram-wire wire-b" /><span className="diagram-arrow arrow-b">›</span><div className="diagram-node home"><span className="node-icon">⌂</span><strong>Locuința</strong><small>consum prioritar</small></div><div className="diagram-branch"><span className="branch-line" /><span className="branch-arrow">↓</span><div className="diagram-node battery"><span className="node-icon">▤</span><strong>Baterie</strong><small>opțional · 2 kWh</small></div></div><div className="diagram-grid"><span className="grid-line" /><span className="grid-arrow">↕</span><div className="diagram-node grid"><span className="node-icon">◎</span><strong>Rețeaua</strong><small>backup / surplus</small></div></div></div></div></div><div className="intro-note"><span>✦</span><p><strong>Ideea simplă:</strong> consumi mai mult din energia pe care o produci tu, exact când ai nevoie de ea.</p></div></section>

      <section className="product-showcase shell"><div className="showcase-copy"><div className="section-label">KITUL COMPLET</div><h2>Tot ce ai nevoie.<br /><em>Într-un singur sistem.</em></h2><p>Panouri full-black, baterie LiFePO4, microinvertor și prinderi pentru balcon. O configurație premium, gândită să arate bine și să lucreze discret.</p><div className="spec-grid"><div><strong>920 Wp</strong><span>panouri</span></div><div><strong>800 W</strong><span>ieșire AC</span></div><div><strong>2 kWh</strong><span>stocare</span></div><div><strong>Wi‑Fi</strong><span>monitorizare</span></div></div><a className="button primary" href="#oferta">Vreau configurația mea <span>↗</span></a></div><div className="showcase-media"><img src="/product-kit.png" alt="Kit solar de balcon cu panouri, baterie, microinvertor și prinderi" /><span className="media-label">Componentă cu componentă · clar de la început</span></div></section>

      <section className="section shell calculator-section" id="calculator"><div className="section-label">02 / ESTIMATOR</div><div className="calc-layout"><div><h2>Cât ai putea economisi?</h2><p className="muted">Mută sliderul și vezi instant cum se schimbă estimarea. Calculul folosește producția solară tipică pentru oraș, o utilizare directă estimată și un tarif orientativ de 1,05 lei/kWh.</p><div className="calc-card"><label htmlFor="consumption">Consum lunar estimat <output>{consumption} kWh</output></label><input id="consumption" type="range" min="80" max="600" step="10" value={consumption} onChange={(e) => setConsumption(Number(e.target.value))} /><div className="range-labels"><span>80 kWh</span><span>600 kWh</span></div><label htmlFor="city">Orașul tău</label><select id="city" value={city} onChange={(e) => setCity(e.target.value)}><option>București</option><option>Constanța</option><option>Craiova</option><option>Cluj-Napoca</option><option>Iași</option><option>Timișoara</option></select></div></div><div className="result-card"><span className="result-kicker">ESTIMARE ANUALĂ · LIVE</span><strong>{estimate.saving.toLocaleString("ro-RO")} lei</strong><p>economisiți prin energia folosită direct în locuință</p><div className="result-line"><span>Producție posibilă</span><b>~{estimate.annualProduction} kWh/an</b></div><div className="result-line"><span>Folosit direct în casă</span><b>~{estimate.selfConsumed} kWh/an</b></div><div className="result-line"><span>Acoperire estimată</span><b>până la {estimate.coverage}%</b></div><a href="#oferta" className="button light">Vreau calculul meu exact <span>↗</span></a><small>Calcul orientativ, construit pe metodologia PVGIS + ipoteze de autoconsum.</small></div></div></section>

      <section className="section shell options" id="optiuni"><div className="section-label">03 / ALEGEREA TA</div><div className="section-heading"><h2>Începi simplu.<br />Extinzi când ești pregătit.</h2><p>Două configurații, aceeași idee: mai multă energie produsă de tine.</p></div><div className="product-grid">{products.map((product) => <article className={`product-card ${product.accent}`} key={product.title}><span className="product-tag">{product.tag}</span><div className="product-visual"><img src="/product-kit.png" alt={`Imagine produs pentru configurația ${product.title.toLowerCase()}`} /></div><h3>{product.title}</h3><p>{product.body}</p><strong className="product-price">{product.price}</strong><ul>{product.items.map((item) => <li key={item}><span>✓</span>{item}</li>)}</ul><a href="#oferta" className="product-link">Solicită o ofertă <span>→</span></a></article>)}</div></section>
